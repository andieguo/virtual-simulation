...
var airquality = {
  ...
  html :  ' <div class="box box-element ui-draggable"> <a href="#close" class="remove label label-important"><i class="icon-remove icon-white"></i>删除</a> <span class="drag label"><i class="icon-move"></i>拖动</span>'+
                '<span class="configuration"><button type="button" class="btn btn-mini" data-target="#attrEditorModal" role="button" data-toggle="modal">编辑</button></span>'+
                 '<div class="preview">空气质量传感器</div>' +
                 '<div class="view">' +
                    '<div class="panel-sensor" id="airquality">'+
                        '<h3 class="title">'+
                          '空气质量传感器标题'+
                          '<div class="right">MAC地址：<span class="mac">11:22:33:44:55:66:77:88</span></div>'+
                        '</h3>'+
                        '<div class="body">'+
                          '<img class="img" src="images/sensor/airquality.jpg" alt="">'+
                          '<div class="button">'+
                          ...
                          '</div>'+
                          '<div class="value">'+
                            '空气质量：<span class="t_value">0</span>ppm&nbsp;&nbsp;'+
  ...
  create: function(){//默认情况下无参数，可接收控件属性参数对象create(properties)
    ...
    properties.tid = "airquality";
    properties.title = "空气质量传感器";
  ...
  if(arguments.length >0){
    ...
    var ui = new AirQualityUI(properties);
    return ui;
  },
  ...
  handSensorBtnEvt:function(key,divid){
    if(uiTemplateObj[divid].power == "on"){
      switch(key){
        case "K1"://增加空气质量浓度值、并立马上报
          if((uiTemplateObj[divid].alist[0].recent_val+100) <= uiTemplateObj[divid].alist[0].data_policy.max_val){

            uiTemplateObj[divid].alist[0].recent_val = parseFloat(uiTemplateObj[divid].alist[0].recent_val)+100;
          ...
          break;
        case "K2"://减少空气质量浓度值、并立马上报
          if((uiTemplateObj[divid].alist[0].recent_val-100) >= uiTemplateObj[divid].alist[0].data_policy.min_val){
